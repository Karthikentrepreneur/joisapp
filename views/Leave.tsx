...
export const Leave: React.FC<LeaveProps> = ({ role, showToast }) => {
  ...
  const handleCreateRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!child) {
      showToast?.("No Student", "error", "No student found on this account.");
      return;
    }
    ...
    const newRequest: LeaveRequest = {
      ...
      endDate: endDate,
      reason: reason,
      // Fix: 'Pending' instead of 'Waiting'
      status: 'Pending',
    ...
    try {
      ...
      setEndDate('');
      setReason('');
      showToast?.("Sent", "success", "Waiting for school to check.");
    } catch (err) {
      showToast?.("Error", "error", "Could not send the request.");
    } finally {
      setIsSubmitting(false);
    }
  };

  // Fix: status to match 'Approved' | 'Rejected'
  const handleAction = async (id: string, status: 'Approved' | 'Rejected') => {
    try {
      await db.update('leaveRequests', id, { status });
      showToast?.(`Answer: ${status}`, "info", `Record has been updated.`);
    } catch (e) {
      showToast?.("Error", "error", "Could not save the answer.");
    }
  };

  // Fix: 'Pending' instead of 'Waiting'
  const filteredRequests = activeTab === 'waiting' ? requests.filter(r => r.status === 'Pending') : requests;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <CalendarDays className="w-6 h-6 text-blue-500" /> Holiday & Sick Leave
          </h2>
          <p className="text-slate-500">{isParent ? "Ask for a day off for your student." : "Check leave requests."}</p>
        </div>
        {isParent && (
          <button onClick={() => setShowModal(true)} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-blue-700 shadow-sm flex items-center gap-2 transition-all"><Plus className="w-4 h-4" /> Ask for Leave</button>
        )}
      </div>

      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 text-slate-400"><Loader2 className="w-10 h-10 animate-spin mb-4" /><p className="font-black uppercase tracking-widest text-xs">Loading...</p></div>
      ) : isParent ? (
        <div className="space-y-6">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {/* Fix: use 'Pending' and 'Approved' */}
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"><p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Waiting</p><h3 className="text-3xl font-black text-yellow-500">{requests.filter(r => r.status === 'Pending').length}</h3></div>
                 ...
                 {requests.map((req) => (
                    <div key={req.id} className="p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
                       <div className="flex items-start gap-4">
                          {/* Fix: status 'Approved', 'Rejected', 'Pending' */}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[calc(100%-80px)] overflow-hidden">
           <div className="border-b border-slate-200 flex">
              {/* Fix: 'Pending' instead of 'Waiting' */}
              <button onClick={() => setActiveTab('waiting')} className={`px-6 py-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'waiting' ? 'border-blue-500 text-blue-600 bg-blue-50/50' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>Waiting {requests.filter(r => r.status === 'Pending').length > 0 && <span className="bg-yellow-500 text-white text-[10px] px-2 py-0.5 rounded-full ml-1">{requests.filter(r => r.status === 'Pending').length}</span>}</button>
           ...
           <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50/30">
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                 {filteredRequests.length === 0 ? (
                    <div className="col-span-full py-20 text-center text-slate-500 font-medium">No {activeTab} requests.</div>
                 ) : (
                    filteredRequests.map((req) => (
                       <div key={req.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                          ...
                          {/* Fix: 'Pending' instead of 'Waiting' and handleAction statuses */}
                          {req.status === 'Pending' && role === UserRole.ADMIN && (
                             <div className="flex gap-2"><button onClick={() => handleAction(req.id, 'Approved')} className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 rounded-lg text-xs font-bold transition-all">Yes</button><button onClick={() => handleAction(req.id, 'Rejected')} className="flex-1 bg-red-500 hover:bg-red-600 text-white py-2 rounded-lg text-xs font-bold transition-all">No</button></div>
                          )}
                       </div>
                    ))
                 )}
      ...
      {showModal && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200 p-4">
            <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl">
               <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-slate-800">Ask for Leave</h3><button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600"><X className="w-6 h-6" /></button></div>
               <form onSubmit={handleCreateRequest} className="space-y-4">
                  ...
                  <div><label className="block text-sm font-medium text-slate-700 mb-1">Reason</label><textarea required value={reason} onChange={e => setReason(e.target.value)} placeholder="Why is the student away?" className="w-full px-4 py-2 border border-slate-200 rounded-xl bg-white h-24 resize-none"></textarea></div>
                  <div className="bg-yellow-50 p-3 rounded-lg flex gap-2 text-xs text-yellow-800 border border-yellow-100"><AlertCircle className="w-4 h-4 shrink-0" /><p>Ask at least 1 day before.</p></div>
                  <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700 shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">{isSubmitting && <Loader2 className="w-4 h-4 animate-spin" />}Send Request</button>
               </form>
            </div>
         </div>
      )}
