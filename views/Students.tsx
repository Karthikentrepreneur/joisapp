...
import { Search, Filter, Plus, Phone, Mail, Bus, X, Save, Pencil, DollarSign, Loader2, Upload, Camera, RefreshCw, Image as ImageIcon } from 'lucide-react';
...
export const Students: React.FC<StudentsProps> = ({ role }) => {
  ...
  const handlePhotoFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        await handlePhotoUpload(reader.result as string);
      };
  ...
  const handleSaveStudent = async (e: React.FormEvent) => {
    ...
    const studentToSave = { 
      ...
      dob: cleanStudentData.dob === '' ? null : cleanStudentData.dob 
    };
    ...
    try {
      if (editingId) {
        ...
        const studentId = `ST-2024-${String(Date.now()).slice(-6)}`;
        const created: Student = {
          ...studentToSave,
          id: studentId,
          parentId: `USR-PARENT-${String(Date.now()).slice(-6)}`,
        } as Student;
        
        await db.create('students', created);
      ...
      alert(`Operation failed: ${err.message || 'Check database connection.'}`);
    } finally {
      setSaving(false);
    }
  };
    ...
    reader.onload = async (event) => {
      ...
      try {
        for (let i = 0; i < dataRows.length; i++) {
          ...
          const [name, grade, section, parentName, parentPhone, parentEmail, dob, busRoute] = columns;
          const studentId = `ST-AUTO-${Date.now()}-${i}`;
          
          const created: Student = {
            id: studentId,
            name: name,
            grade: grade || 'Play Group',
            section: section || 'A',
            parentName: parentName,
            parentPhone: parentPhone,
            parentEmail: parentEmail,
            parentId: `USR-PARENT-AUTO-${Date.now()}-${i}`,
            attendance: 100,
            feesStatus: 'Pending',
            busRoute: busRoute || 'Parent Pickup',
            image: `https://api.dicebear.com/7.x/avataaars/svg?seed=${name}`,
            dob: (dob && dob !== '') ? dob : null as any
          };
        ...
        await loadStudents();
        alert(`Imported ${dataRows.length} students.`);
      } catch (err: any) {
        alert(`Bulk upload partial failure: ${err.message}`);
      } finally {
        setSaving(false);
    ...
    <div className="h-full overflow-y-auto p-4 md:p-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage preschool groups and child profiles.</p>
        </div>
        {role === UserRole.ADMIN && (
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={saving} className="bg-white text-slate-700 px-4 py-2 rounded-lg text-sm font-medium border border-slate-200 hover:bg-slate-50 shadow-sm flex items-center gap-2 justify-center flex-1 md:flex-none">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4 text-blue-500" />}
              Bulk Upload
            </button>
            <button onClick={() => setShowAddModal(true)} disabled={saving} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 shadow-sm flex items-center gap-2 justify-center flex-1 md:flex-none">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 text-slate-400">
          ...
          <p className="font-medium">Fetching records from database...</p>
        </div>
      ) : (
          ...
          {filteredStudents.map(student => (
            <div key={student.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden group">
              ...
              <div className="p-4 bg-slate-50/50 space-y-2">
                <div className="flex items-center gap-3 text-sm"><Phone className="w-4 h-4 text-slate-400" /><span className="text-slate-600 font-medium">{student.parentPhone}</span></div>
                <div className="flex items-center gap-3 text-sm"><Mail className="w-4 h-4 text-slate-400" /><span className="text-slate-600 font-medium truncate max-w-full" title={student.parentEmail}>{student.parentEmail || 'No email'}</span></div>
              </div>
              {role === UserRole.ADMIN && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
           <div className="bg-white rounded-3xl p-6 md:p-8 max-w-2xl w-full shadow-2xl overflow-y-auto max-h-[90vh]">
              <div className="flex justify-between items-center mb-6">
                 ...
                 <button onClick={handleCloseModal} className="text-slate-400 hover:text-slate-600" disabled={saving}><X className="w-6 h-6" /></button>
              </div>
                    ...
                    <div className="space-y-4">
                             ...
                             {showCamera ? (
                                ...
                                <>
                                   ...
                                   <div className="absolute -bottom-2 -right-2 flex flex-col gap-2">
                                      ...
                                      <input type="file" ref={photoFileInputRef} onChange={handlePhotoFileChange} accept="image/*" className="hidden" />
                                   </div>
                                </>
                             )}
                          </div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Passport Photo</p>
                          <canvas ref={canvasRef} className="hidden" />
                       </div>

                       <div>
                          <label className="block text-sm font-medium text-slate-700 mb-1">Full Name</label>
                          <input required type="text" value={newStudent.name} onChange={e => setNewStudent({...newStudent, name: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                       </div>
                       <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <select value={newStudent.section} onChange={e => setNewStudent({...newStudent, section: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl bg-white"><option>A</option><option>B</option></select>
                        </div>
                       </div>
                       <div>
                          <label className="block text-sm font-medium text-slate-700 mb-1 flex items-center gap-2"><RefreshCw className="w-4 h-4 text-slate-400" /> Date of Birth</label>
                          <input type="date" value={newStudent.dob} onChange={e => setNewStudent({...newStudent, dob: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                       </div>
                       <h4 className="font-bold text-slate-700 text-xs uppercase tracking-wider mt-6 mb-2 text-blue-600">Parent / Guardian</h4>
                       <div>
                          <label className="block text-sm font-medium text-slate-700 mb-1">Parent Full Name</label>
                          <input required type="text" value={newStudent.parentName} onChange={e => setNewStudent({...newStudent, parentName: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl bg-white" />
                       </div>
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div><label className="block text-sm font-medium text-slate-700 mb-1">Primary Phone</label><input required type="tel" value={newStudent.parentPhone} onChange={e => setNewStudent({...newStudent, parentPhone: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl bg-white" /></div>
                        <div><label className="block text-sm font-medium text-slate-700 mb-1">Email</label><input required type="email" value={newStudent.parentEmail} onChange={e => setNewStudent({...newStudent, parentEmail: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl bg-white" /></div>
                       </div>
                    </div>
                    <div className="space-y-4">
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                            <h4 className="font-bold text-blue-800 text-xs uppercase tracking-wider mb-3 flex items-center gap-2"><DollarSign className="w-4 h-4" /> Fees</h4>
                            <div className="space-y-3">
                                <div><label className="block text-xs font-bold text-slate-500 mb-1">Total Amount (₹)</label><input type="number" value={newStudent.feeAmount} onChange={e => setNewStudent({...newStudent, feeAmount: Number(e.target.value)})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" /></div>
                                <div><label className="block text-xs font-bold text-slate-500 mb-1">Paid Amount (₹)</label><input type="number" value={newStudent.paidAmount} onChange={e => setNewStudent({...newStudent, paidAmount: Number(e.target.value)})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" /></div>
                            </div>
                        </div>
