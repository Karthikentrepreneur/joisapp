...
/* --- PARENT DASHBOARD (Enhanced Aesthetics) --- */
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div 
          ...
          className="group relative bg-white p-8 rounded-[2rem] shadow-sm hover:shadow-xl border border-slate-100 transition-all cursor-pointer overflow-hidden"
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-full w-fit">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-pink-700 bg-pink-50 px-3 py-1.5 rounded-full w-fit">
              Due this week
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="w-14 h-14 bg-yellow-100 text-yellow-600 rounded-2xl flex items-center justify-center mb-6 shadow-sm">
            ...
            {pendingFee > 0 ? (
              <button className="mt-4 w-full bg-slate-900 text-white text-xs font-black py-2.5 rounded-xl hover:bg-slate-800 transition-colors shadow-lg shadow-slate-200 uppercase tracking-widest">
                Pay Now
              </button>
            ) : (
              <div className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-green-700 bg-green-50 px-3 py-1.5 rounded-full w-fit">
                All Cleared
              </div>
            )}
          </div>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-purple-700 bg-purple-50 px-3 py-1.5 rounded-full w-fit">
              Apply online
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Child's Timetable (Preschool Schedule) */}
           ...
           <div className="space-y-6">
              {[
                ...
                <div key={i} className={`flex items-center gap-6 p-5 rounded-[2rem] transition-all duration-300 ${slot.status === 'active' ? 'bg-blue-50/50 border-2 border-blue-200 shadow-md ring-4 ring-blue-50' : 'bg-slate-50 hover:bg-slate-100 border border-transparent'}`}>
                   ...
                   <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-sm ${slot.color}`}>
                     {slot.icon}
                   </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              ...
              {myLeaveRequests.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                   ...
                   <div>
                     ...
                     <p className="text-xs text-slate-300 mt-1">History will appear here</p>
                   </div>
                </div>
              ) : (
                myLeaveRequests.map((req) => (
                  <div key={req.id} className="p-5 bg-slate-50 rounded-3xl border border-slate-100 hover:border-purple-200 transition-colors">
                     ...
                     <p className="text-base font-black text-slate-800 flex items-center gap-2">
                        {req.startDate} <ArrowRight className="w-3 h-3 text-slate-400" /> {req.endDate}
                     </p>
           ...
           <button 
             onClick={() => setShowLeaveModal(true)}
             className="w-full mt-8 bg-slate-50 hover:bg-slate-100 text-slate-600 font-bold py-4 rounded-2xl border border-slate-100 flex items-center justify-center gap-2 transition-all group"
           >
             Apply for Leave <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform" />
           </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md animate-in fade-in duration-300 p-4">
           <div className="bg-white rounded-[2.5rem] p-8 md:p-10 max-w-md w-full shadow-2xl border border-white/20">
              <div className="flex justify-between items-center mb-8">
                 ...
                 <button onClick={() => setShowLeaveModal(false)} className="w-10 h-10 bg-slate-50 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full flex items-center justify-center transition-colors">
                    <X className="w-6 h-6" />
                 </button>
              </div>
              ...
              <form onSubmit={handleLeaveSubmit} className="space-y-5">
                 <div className="space-y-1.5">
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Start Date</label>
                    <input 
                      type="date" 
                      required
                      value={leaveStartDate}
                      onChange={(e) => setLeaveStartDate(e.target.value)}
                      className="w-full px-5 py-3.5 border border-slate-200 rounded-2xl text-sm font-semibold focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all bg-white"
                    />
                 </div>
                 <div className="space-y-1.5">
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest ml-1">End Date</label>
                    <input 
                      type="date" 
                      required
                      value={leaveEndDate}
                 ...
                 <div className="space-y-1.5">
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Reason</label>
                    <textarea 
                      required
                      value={leaveReason}
                      onChange={(e) => setLeaveReason(e.target.value)}
                      placeholder="e.g., Family event, medical appointment..."
                      className="w-full px-5 py-3.5 border border-slate-200 rounded-2xl text-sm font-semibold focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all bg-white h-28 resize-none"
                    ></textarea>
                 </div>

                 <button 
                    type="submit"
                    className="w-full bg-blue-600 text-white py-4 rounded-[1.5rem] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-200 mt-4 active:scale-95"
                 >
                    Submit Request
                 </button>
              </form>
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
         <div>
            <h1 className="text-3xl font-black text-slate-900 leading-tight">Class 5-A Overview</h1>
            <p className="text-slate-500 font-medium">Welcome back, {currentTeacher.name}! You have <span className="text-pink-600 font-bold">2 assignments</span> to review.</p>
         </div>
         <div className="flex flex-wrap gap-3 w-full md:w-auto">
            <button 
              onClick={() => setShowPayslip(true)}
              className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-6 py-3 rounded-2xl text-sm font-black uppercase tracking-widest transition-all flex items-center gap-2 flex-1 md:flex-none justify-center"
            >
               <FileText className="w-4 h-4" /> My Payslip
            </button>
            <button 
              onClick={() => onNavigate(View.ACADEMICS)}
              className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl text-sm font-black uppercase tracking-widest transition-all flex items-center gap-2 flex-1 md:flex-none justify-center shadow-lg shadow-blue-100"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <div 
           onClick={() => onNavigate(View.ATTENDANCE)}
           className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl transition-all cursor-pointer group relative overflow-hidden"
         >
            ...
            <div className="relative z-10">
              <div className="w-14 h-14 bg-blue-100 rounded-2xl flex items-center justify-center text-blue-600 mb-6 group-hover:rotate-12 transition-transform shadow-sm">
                 <CheckCircle className="w-7 h-7" />
              </div>
         ...
         >
            ...
            <div className="relative z-10">
              <div className="w-14 h-14 bg-pink-100 rounded-2xl flex items-center justify-center text-pink-600 mb-6 group-hover:rotate-12 transition-transform shadow-sm">
                 <BookOpen className="w-7 h-7" />
              </div>
         ...
         >
            ...
            <div className="relative z-10">
              <div className="w-14 h-14 bg-yellow-100 rounded-2xl flex items-center justify-center text-yellow-600 mb-6 group-hover:rotate-12 transition-transform shadow-sm">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
         <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest">
               <div className="w-3 h-3 bg-blue-500 rounded-full"></div> Average Score
            </div>
         </div>
         <div className="h-72">
           <ResponsiveContainer width="100%" height="100%">
              <BarChart data={[
                 ...
                 <Bar dataKey="avg" radius={[12, 12, 0, 0]} barSize={48}>
                    <Cell fill="#3b82f6" />
                    <Cell fill="#10b981" />
                    <Cell fill="#ec4899" />
                    <Cell fill="#f59e0b" />
           ...
           <div id="payslip-modal-content" className="bg-white rounded-[2.5rem] shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto print:max-h-none print:shadow-none print:w-full print:rounded-none">
              <div className="p-8 md:p-12 bg-white text-slate-900">
                ...
                <div className="text-center mb-10">
                   <h2 className="text-xl font-black text-slate-900 uppercase inline-block border-b-4 border-slate-900 pb-1">Salary Slip</h2>
                ...
                <div className="mb-10 text-sm">
                   <div className="grid grid-cols-2 gap-y-6 gap-x-12">
                      <div>
                         <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest mb-1">Employee Name</p>
                         <p className="font-black text-slate-900 text-base">{currentTeacher.name}</p>
                      </div>
                      <div>
                         <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest mb-1">Designation</p>
                         <p className="font-black text-slate-900 text-base">{currentTeacher.role}</p>
                      </div>
                      <div>
                         <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest mb-1">Employee ID</p>
                         <p className="font-black text-slate-900 text-base">{currentTeacher.id}</p>
                      </div>
                      <div>
                         <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest mb-1">Pay Date</p>
                         <p className="font-black text-slate-900 text-base">31 March, 2024</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="border-2 border-slate-100 rounded-[2rem] overflow-hidden mb-8 text-sm">
                   ...
                   <div className="p-6 space-y-4 border-b border-slate-100">
                      <div className="flex justify-between items-center">
                         <span className="text-slate-600 font-bold uppercase tracking-wide text-xs">Basic Salary</span>
                         <span className="font-black text-slate-900">₹{currentTeacher.salaryDetails?.basic?.toLocaleString() || '0'}</span>
                      </div>
                      <div className="flex justify-between items-center">
                         <span className="text-slate-600 font-bold uppercase tracking-wide text-xs">Allowances (HRA/DA)</span>
                         <span className="font-black text-slate-900">₹{currentTeacher.salaryDetails?.allowances?.toLocaleString() || '0'}</span>
                      </div>
                   </div>
                   ...
                   <div className="p-6 space-y-4 border-b border-slate-100">
                      <div className="flex justify-between items-center">
                         <span className="text-slate-600 font-bold uppercase tracking-wide text-xs">Tax / PF</span>
                         <span className="font-black text-slate-900">₹{currentTeacher.salaryDetails?.deductions?.toLocaleString() || '0'}</span>
                      </div>
                   </div>
                </div>

                <div className="bg-slate-900 rounded-[2rem] p-8 flex justify-between items-center mb-16 shadow-xl shadow-slate-200">
                   <span className="font-black text-white text-xl uppercase tracking-tighter">Net Pay</span>
                ...
                <div className="flex justify-between items-end mt-12 px-6">
                   <div className="flex flex-col items-center">
                      <div className="h-20 flex items-end justify-center mb-4">
                         {adminStaff?.signature && <img src={adminStaff.signature} alt="Sig" className="h-16 w-32 object-contain mix-blend-multiply" />}
                      </div>
                      <div className="w-40 border-t-2 border-slate-900 pt-3 text-center">
                         <p className="font-black text-slate-900 text-[10px] uppercase tracking-widest">Principal</p>
                      </div>
                   </div>
                   <div className="flex flex-col items-center">
                      <div className="h-20 flex items-end justify-center mb-4">
                         {currentTeacher?.signature && <img src={currentTeacher.signature} alt="Sig" className="h-16 w-32 object-contain mix-blend-multiply" />}
                      </div>
                      <div className="w-40 border-t-2 border-slate-900 pt-3 text-center">
                         <p className="font-black text-slate-900 text-[10px] uppercase tracking-widest">Employee</p>
                      </div>
                   </div>
                </div>
              </div>
              ...
              <div className="p-6 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-4 bg-slate-50 no-print">
                 <button className="flex items-center justify-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-2xl text-slate-700 hover:bg-slate-100 font-bold uppercase tracking-widest text-xs transition-all" onClick={() => window.print()}>
                    <Printer className="w-4 h-4" /> Print PDF
                 </button>
                 <button onClick={() => setShowPayslip(false)} className="flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl hover:bg-black font-bold uppercase tracking-widest text-xs transition-all shadow-lg">
                    Close
                 </button>
              </div>
           </div>
...
const AdminDashboard = ({ onNavigate }: { onNavigate: (view: View) => void }) => {
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-3xl font-black text-slate-900 leading-tight">Admin Console</h1>
          <p className="text-slate-500 font-medium">School-wide performance monitoring and operations.</p>
        </div>
        <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-2xl px-5 py-3 shadow-sm">
           ...
           <span className="text-sm font-black text-emerald-700 uppercase tracking-widest">System Healthy</span>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div onClick={() => onNavigate(View.STUDENTS)} className="cursor-pointer">
          <StatCard icon={Users} label="Total Students" value={mockStudents.length.toString()} trend="+5 New" color="blue" />
        </div>
        <div onClick={() => onNavigate(View.STAFF)} className="cursor-pointer">
          <StatCard icon={Users} label="Total Staff" value={mockStaff.length.toString()} trend="All Present" color="purple" />
        </div>
        <div onClick={() => onNavigate(View.FEES)} className="cursor-pointer">
          <StatCard icon={DollarSign} label="Revenue" value="₹42.5L" trend="+8% vs LY" color="yellow" />
        </div>
        <div onClick={() => onNavigate(View.SAFETY)} className="cursor-pointer">
          <StatCard icon={AlertTriangle} label="Safety Alerts" value="0" trend="All Clear" color="emerald" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={schoolData}>
                  ...
                  <Area type="monotone" dataKey="fees" stroke="#3b82f6" strokeWidth={4} fillOpacity={1} fill="url(#colorAtt)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
         </div>
         ...
         <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
            ...
            <div className="space-y-4">
               {[
                 { label: 'Admit New Student', view: View.STUDENTS, icon: Users, color: 'bg-blue-50 text-blue-600', hover: 'hover:border-blue-200' },
                 { label: 'Generate Invoices', view: View.FEES, icon: DollarSign, color: 'bg-yellow-50 text-yellow-600', hover: 'hover:border-yellow-200' },
                 { label: 'Update Schedule', view: View.ACADEMICS, icon: Calendar, color: 'bg-pink-50 text-pink-600', hover: 'hover:border-pink-200' },
                 { label: 'System Reports', view: View.DOCUMENTS, icon: FileText, color: 'bg-purple-50 text-purple-600', hover: 'hover:border-purple-200' }
               ].map((action, i) => (
                  <button 
                    key={i}
                    onClick={() => onNavigate(action.view)}
                    className={`w-full text-left p-5 rounded-[1.5rem] border border-slate-100 ${action.hover} transition-all flex items-center gap-4 hover:shadow-md hover:bg-slate-50 group`}
                  >
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
         <div className="flex justify-between items-center mb-8">
             <h3 className="text-2xl font-black text-slate-900">Pending Leave Requests</h3>
             <span className="bg-yellow-100 text-yellow-700 text-xs font-black px-4 py-2 rounded-full uppercase tracking-widest">{pendingLeaves.length} To Action</span>
         </div>
                ...
                <div key={req.id} className="border-2 border-slate-50 rounded-[2rem] p-6 hover:border-blue-100 transition-all bg-slate-50/50 group">
                   <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                         <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 font-black text-xs border border-slate-100 shadow-sm group-hover:text-blue-500 transition-colors">
                            {req.studentName.charAt(0)}
                         </div>
                         <div>
                            <h4 className="font-black text-slate-800 leading-tight">{req.studentName}</h4>
                            <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">{req.studentId}</p>
                         </div>
                      </div>
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{req.requestDate}</span>
                   </div>
                   <div className="mb-6">
                      <div className="inline-flex items-center gap-2 text-xs font-black text-slate-700 mb-2 bg-white px-3 py-1.5 rounded-full border border-slate-100">
                        <Calendar className="w-3 h-3 text-blue-500" />
                        {req.startDate} <span className="text-slate-300">→</span> {req.endDate}
                      </div>
                      <p className="text-xs text-slate-500 mt-2 italic font-medium leading-relaxed bg-white/50 p-3 rounded-2xl border border-white">"{req.reason}"</p>
                   </div>
                   <div className="flex gap-3">
                      <button 
                        onClick={() => handleLeaveAction(req.id, 'Approved')}
                        className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 transition-all active:scale-95"
                      >
                         <Check className="w-4 h-4 stroke-[3]" /> Approve
                      </button>
                      <button 
                        onClick={() => handleLeaveAction(req.id, 'Rejected')}
                        className="flex-1 bg-rose-500 hover:bg-rose-600 text-white py-3 rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-rose-100 flex items-center justify-center gap-2 transition-all active:scale-95"
                      >
...
const StatCard = ({ icon: Icon, label, value, trend, color }: any) => {
  ...
  return (
    <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:translate-y-[-4px] transition-all group overflow-hidden relative">
      <div className={`absolute top-0 right-0 w-2 h-full opacity-0 group-hover:opacity-100 transition-opacity ${colorClasses[color].split(' ')[1]}`}></div>
      <div className="flex items-center justify-between mb-6">
        <div className={`p-4 rounded-2xl shadow-sm ${colorClasses[color]}`}>
