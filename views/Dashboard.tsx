...
const ParentDashboard = ({ onNavigate }: { onNavigate: (view: View) => void }) => {
  ...
  const handleLeaveSubmit = (e: React.FormEvent) => {
    ...
    setLeaveStartDate('');
    // Fix typo: changed setEndDate to setLeaveEndDate to match state definition
    setLeaveEndDate('');
    setLeaveReason('');
    ...
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <div onClick={() => onNavigate(View.COMMUNICATION)} className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl transition-all cursor-pointer group relative overflow-hidden">
            <div className="absolute -right-4 -top-4 w-20 h-20 bg-yellow-50 rounded-full group-hover:scale-150 transition-transform duration-500"></div>
            <div className="relative z-10">
      ...
      {/* Salary Slip Modal with Compact Printable Layout */}
      {showPayslip && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 print:p-0">
           <div id="payslip-modal-content" className="bg-white rounded-[2rem] shadow-2xl max-w-2xl w-full max-h-[95vh] overflow-y-auto print:max-h-none print:shadow-none print:w-full print:rounded-none">
              <div className="p-6 md:p-10 bg-white text-slate-900 relative">
                {/* No icon in top right corner for professional appearance */}
                
                <div className="flex flex-col items-center text-center mb-4">
                   <img src="https://www.joischools.com/assets/jois-logo-BUnvOotz.png" alt="Logo" className="h-16 w-auto mb-2" />
                   <h1 className="text-xl font-black uppercase tracking-tight text-slate-900 leading-tight">Junior Odyssey International School</h1>
                   <div className="text-[10px] font-bold text-slate-400 mt-1 leading-relaxed uppercase tracking-widest">
                ...
                <div className="text-center mb-6">
                   <h2 className="text-lg font-black text-slate-900 uppercase inline-block border-b-2 border-slate-900 pb-0.5 tracking-tight">Official Salary Slip</h2>
                   <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-widest">March, 2024</p>
                </div>

                <div className="mb-6 text-sm">
                   <div className="grid grid-cols-2 gap-y-3 gap-x-8">
                      <div><p className="text-slate-400 font-black text-[9px] uppercase tracking-widest mb-0.5">Employee Name</p><p className="font-black text-slate-900 text-sm">{currentTeacher.name}</p></div>
                      <div><p className="text-slate-400 font-black text-[9px] uppercase tracking-widest mb-0.5">Designation</p><p className="font-black text-slate-900 text-sm">{currentTeacher.role}</p></div>
                      <div><p className="text-slate-400 font-black text-[9px] uppercase tracking-widest mb-0.5">Employee ID</p><p className="font-black text-slate-900 text-sm">{currentTeacher.id}</p></div>
                      <div><p className="text-slate-400 font-black text-[9px] uppercase tracking-widest mb-0.5">Pay Date</p><p className="font-black text-slate-900 text-sm">31 March, 2024</p></div>
                   </div>
                </div>

                <div className="border border-slate-100 rounded-2xl overflow-hidden mb-6 text-sm">
                   <div className="bg-slate-50 px-4 py-2 font-black text-[9px] uppercase tracking-widest text-slate-400 border-b border-slate-100">Earnings</div>
                   <div className="p-4 space-y-2 border-b border-slate-100">
                      <div className="flex justify-between items-center"><span className="text-slate-600 font-bold uppercase tracking-wide text-[11px]">Basic Salary</span><span className="font-black text-slate-900 text-sm">₹{currentTeacher.salaryDetails?.basic?.toLocaleString() || '0'}</span></div>
                      <div className="flex justify-between items-center"><span className="text-slate-600 font-bold uppercase tracking-wide text-[11px]">Allowances (HRA/DA)</span><span className="font-black text-slate-900 text-sm">₹{currentTeacher.salaryDetails?.allowances?.toLocaleString() || '0'}</span></div>
                   </div>
                   <div className="bg-slate-50 px-4 py-2 font-black text-[9px] uppercase tracking-widest text-slate-400 border-b border-slate-100">Deductions</div>
                   <div className="p-4 border-b border-slate-100"><div className="flex justify-between items-center"><span className="text-slate-600 font-bold uppercase tracking-wide text-[11px]">Tax / PF</span><span className="font-black text-slate-900 text-sm">₹{currentTeacher.salaryDetails?.deductions?.toLocaleString() || '0'}</span></div></div>
                </div>

                <div className="bg-slate-900 rounded-2xl p-6 flex justify-between items-center mb-8 shadow-md">
                   <span className="font-black text-white text-base uppercase tracking-tight">Net Payable</span>
                   <span className="font-black text-white text-3xl">₹{currentTeacher.salaryDetails?.net?.toLocaleString() || '0'}</span>
                </div>
                
                <div className="flex justify-between items-end mt-4 px-4">
                   <div className="flex flex-col items-center">
                      <div className="h-20 flex items-end justify-center mb-2">
                         {adminStaff?.signature && <img src={adminStaff.signature} alt="Principal Signature" className="h-14 w-auto object-contain" />}
                      </div>
                      <div className="w-40 border-t border-slate-900 pt-2 text-center">
                         <p className="font-black text-slate-900 text-[9px] uppercase tracking-widest">Principal</p>
                         <p className="text-[9px] text-slate-400 font-bold tracking-tighter uppercase">{adminStaff?.name}</p>
                      </div>
                   </div>
                   <div className="flex flex-col items-center">
                      <div className="h-20 flex items-end justify-center mb-2">
                         {currentTeacher?.signature && <img src={currentTeacher.signature} alt="Employee Signature" className="h-14 w-auto object-contain" />}
                      </div>
                      <div className="w-40 border-t border-slate-900 pt-2 text-center">
                         <p className="font-black text-slate-900 text-[9px] uppercase tracking-widest">Employee</p>
                         <p className="text-[9px] text-slate-400 font-bold tracking-tighter uppercase">{currentTeacher?.name}</p>
                      </div>
                   </div>
                </div>

                <div className="text-center mt-10 pt-4 border-t border-slate-50 print:block hidden">
                   <p className="text-[9px] text-slate-400 italic">This is a computer-generated document and requires both authorized signatures to be valid.</p>
                </div>
              </div>

              <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3 bg-slate-50 no-print">
                 <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-xl text-slate-700 hover:bg-slate-50 font-black uppercase tracking-widest text-[11px] transition-all shadow-sm active:scale-95" onClick={() => window.print()}>
                    <Printer className="w-4 h-4 text-blue-500" /> Print
                 </button>
                 <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-black uppercase tracking-widest text-[11px] transition-all shadow-lg shadow-blue-100 active:scale-95" onClick={() => window.print()}>
                    <Download className="w-4 h-4" /> Download
                 </button>
                 <button onClick={() => setShowPayslip(false)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-slate-100 text-slate-500 rounded-xl hover:bg-slate-200 font-black uppercase tracking-widest text-[11px] transition-all">
                    Close
                 </button>
              </div>
           </div>
