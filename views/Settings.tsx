    ...
    <div className="p-4 md:p-6 h-full overflow-y-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage account details and system access.</p>
        </div>
        {role === UserRole.ADMIN && (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
               <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors cursor-pointer relative group h-64">
                  ...
                  <h4 className="font-bold text-slate-700 text-lg">New Signature</h4>
                  <p className="text-sm text-slate-400 mt-2">Click to browse or drag file here</p>
               </div>
               
               <div className="bg-slate-50 rounded-xl p-6 border border-slate-200 flex flex-col items-center justify-center h-64 relative">
                  <div className="absolute top-4 left-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Preview</div>
                  {signaturePreview ? (
                     ...
                     <div className="flex flex-col items-center justify-center text-slate-400">
                        ...
                        <p className="italic">No signature found.</p>
                     </div>
                  )}
               </div>
            </div>
        ...
        {role === UserRole.ADMIN && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
             <div className="p-6 border-b border-slate-100">
                <h3 className="font-bold text-slate-800 text-lg mb-2">Who sees what?</h3>
                <p className="text-sm text-slate-500">
                   Enable or disable specific sections for different user roles.
                </p>
             </div>
                ...
                <table className="w-full text-left border-collapse">
                   <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                         <th className="p-4 font-bold text-slate-700 text-sm">Section Name</th>
                         {roles.map(role => (
